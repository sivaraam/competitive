...
public class NeighbourSumArray_V2_Test {

    @Test
    void testEmptyArray() {
        List<Double> nums = Collections.emptyList();
        assertEquals(0, NeighbourSumArray_V2.neighbourSum(nums));
    }

    @Test
    void testSingleElementArray() {
        List<Double> nums = Collections.singletonList(1d);
        assertEquals(1, NeighbourSumArray_V2.neighbourSum(nums));
    }

    @Test
    void testTwoElementArray() {
        List<Double> nums = Arrays.asList(1d, 4d);
        assertEquals(10, NeighbourSumArray_V2.neighbourSum(nums));
    }

    @Test
    void testThreeElementArray() {
        List<Double> nums = Arrays.asList(1d, 4d, 7d);
        assertEquals(28, NeighbourSumArray_V2.neighbourSum(nums));
    }

    @Test
    void testNegativeNumbers() {
        List<Double> nums = Arrays.asList(-1d, -2d, -3d);
        assertEquals(-14, NeighbourSumArray_V2.neighbourSum(nums));
    }

    @Test
    void testDecimalNumbers() {
        List<Double> nums = Arrays.asList(0.1, 0.2, 0.3);
        assertEquals(1.4, NeighbourSumArray_V2.neighbourSum(nums), 0.0001);
    }

    @Test
    void testLargeNumbers() {
        List<Double> nums = Arrays.asList(1d, -20d, 300d, -4000d, 50000d, -600000d, 7000000d);
        assertEquals(12338842, NeighbourSumArray_V2.neighbourSum(nums));
    }
